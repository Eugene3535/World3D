...
template<size_t N>
std::array<uint32_t, N> GlBufferHolder::generateBuffers() noexcept
{
...
template<size_t N>
void GlBufferHolder::deleteBuffers(const std::array<uint32_t, N>& buffers) noexcept
{
    if(m_instance && m_instance->delBuffers)
    {
//  Avoid deleting non-existent buffers
