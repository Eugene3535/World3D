...
bool Image::loadFromFile(const std::filesystem::path& filepath) noexcept
{
    ...
    uint8_t* data = stbi_load(filepath.generic_string().c_str(), &width, &height, &bytePerPixel, STBI_rgb_alpha);
...
void Image::copy(const Image& source, uint32_t destX, uint32_t destY, const glm::ivec4& sourceRect, bool applyAlpha) noexcept
{
    ...
    if (srcRect.z == 0 || (srcRect.w == 0))
    {
        srcRect.x   = 0;
        srcRect.y    = 0;
        srcRect.z  = static_cast<int>(source.m_size.x);
        srcRect.w = static_cast<int>(source.m_size.y);
    }
    else
    {
        if (srcRect.x < 0) srcRect.x = 0;
        if (srcRect.y < 0) srcRect.y  = 0;
        if (srcRect.z > static_cast<int>(source.m_size.x)) srcRect.z  = static_cast<int>(source.m_size.x);
    ...
    const uint8_t* srcPixels = &source.m_pixels[0] + (static_cast<uint32_t>(srcRect.x) + static_cast<uint32_t>(srcRect.y) * source.m_size.x) * 4;
